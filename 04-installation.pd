...
Next, we have to install the `python3` package. For example, on Ubuntu, you can use [`sudo apt-get install python3`](http://packages.ubuntu.com/search?keywords=python3&searchon=names&suite=all&section=all). Please check the documentation or forums of the Linux distribution that you have installed for the correct package manager command to run.
...
:   Newer distributions such as [Ubuntu 12.10 are making Python 3 the default version](https://wiki.ubuntu.com/Python/3), so check if it is already installed.
