...
In this program, we take guesses from the user and check if it is the number that we have. We set the variable `number` to any integer we want, say `23`. Then, we take the user's guess using the `input()` function. Functions are just reusable pieces of programs. We'll read more about them in the ??[[Python_en:Functions|next chapter]].
...
There is no `switch` statement in Python. You can use an `if..elif..else` statement to do the same thing (and in some cases, use a ?? [[Python_en:Data Structures#Dictionary|dictionary]] to do it quickly)
...
The `for..in` statement is another looping statement which *iterates* over a sequence of objects i.e. go through each item in a sequence. We will see more about ??[[Python_en:Data Structures#Sequences|sequences]] in detail in later chapters. What you need to know right now is that a sequence is just an ordered collection of items.
...
Note that range() generates a sequence of numbers, but it will generate only one number at a time, when the for loop requests for the next item. If you want to see the full sequence of numbers immediately, use list(range()). Lists are explained in the ??[[Python_en:Data Structures|Data Structures chapter]].
...
Remember that the `else` part is optional. When included, it is always executed once after the `for` loop is over unless a ???[[#The break Statement|break]] statement is encountered.
